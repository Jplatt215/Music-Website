...
def index():
    try:
        ...
        composers = sorted(composers, key=lambda c: c.name.split()[-1].lower())
        composers_data = []
        ...
        flash(f"Error loading index: {e}", category="error")
        return render_template('index.html', composers_data=[], user=current_user)

def infoFinder(videoTitle):
    for index, char in enumerate(videoTitle):
        if char in [":", "-"]:
            return videoTitle[:index].strip(), videoTitle[index + 1:].strip()
...
def confirmComposition():
    ...
    if not composer_name or not composition_title or not url:
        flash("Composer, composition, and URL fields are required.", category="error")
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Error saving composition: {e}", category="error")
...
def edit_composer():
    try:
        ...
        composer = Composer.query.get(data['id'])
        if not composer:
            return jsonify(success=False, message="Composer not found"), 404

        if data.get('name'):
            composer.name = data['name'].strip()
        if data.get('lifetime'):
            composer.lifetime = data['lifetime']
...
def edit_composition():
    try:
        ...
        if not data or 'id' not in data:
            return jsonify(success=False, message="Missing composition ID"), 400

        composition = Composition.query.get(data['id'])
        if not composition:
        ...
        if data.get('year'):
            composition.year = data['year']
        if data.get('url'):
            composition.url = data['url'].strip()
...
def delete_composer():
    try:
        ...
        if not data or 'id' not in data:
            return jsonify(success=False, message="Missing composer ID"), 400
...
def save_composition():
    try:
        ...
        if len(title) > 255:
            return jsonify({'success': False, 'message': 'Title must be under 255 characters'}), 400
        if not composition_data:
            return jsonify({'success': False, 'message': 'Composition data is required'}), 400
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': 'Failed to save composition. Please try again.'}), 500
...
def update_composition(slug):
    try:
        ...
        if 'is_public' in data:
            composition.is_public = bool(data['is_public'])
        if 'data' in data:
            if not data['data']:
                return jsonify({'success': False, 'message': 'Composition data is required'}), 400
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': 'Failed to update composition. Please try again.'}), 500
...
def delete_user_composition(slug):
    try:
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': 'Failed to delete composition. Please try again.'}), 500